// ============================================
// modules/editorSystem.js - Éditeur de texte
// ============================================

export const editorSystem = {
    createTextEditor(filename, filePath) {
        ...
        editor.style.zIndex = this.nextZIndex++;
        
        editor.innerHTML = this._buildEditorHTML(editorId, filename);
        
        document.body.appendChild(editor);
        this.makeDraggable(editor);
        this.makeResizable(editor);
        this.makeClickToFront(editor);
        ...
        this.activeWindows.push(windowData);
        
        if (!isMobile) {
            this.cascadeWindow(editor);
        }
        
        this.loadTextFileContent(editorId, filePath);
        this.setupEditorEventListeners(editorId);
        
        return editorId;
    },

    async loadTextFileContent(editorId, filePath) {
        const windowData = this.activeWindows.find(w => w.id === editorId);
        ...
        try {
            ...
            this.updateEditorStatus(editorId, 'Prêt', 'success');
            textarea.focus();
            
        } catch (error) {
            console.error('Erreur de chargement:', error);
            this.updateEditorStatus(editorId, 'Erreur de chargement', 'error');
    ...
    setupEditorEventListeners(editorId) {
        const windowData = this.activeWindows.find(w => w.id === editorId);
        if (!windowData) return;
        ...
        textarea.addEventListener('input', () => {
            ...
            this.updateEditorSaveButton(editorId);
            this.updateEditorStats(editorId);
        });
        ...
        textarea.addEventListener('keyup', () => this.updateEditorCursorPosition(editorId));
        textarea.addEventListener('click', () => this.updateEditorCursorPosition(editorId));
    ...
    updateEditorSaveButton(editorId) {
        const windowData = this.activeWindows.find(w => w.id === editorId);
        if (!windowData) return;
        ...
        this.updateEditorStatus(editorId, windowData.isModified ? 'Modifications non enregistrées' : 'Enregistré', windowData.isModified ? 'warning' : 'success');
    },

    updateEditorStatus(editorId, message, type = 'info') {
    ...
    updateEditorCursorPosition(editorId) {
        const textarea = document.getElementById(editorId + '-textarea');
    ...
    changeEditorFontSize(editorId, size) {
        const textarea = document.getElementById(editorId + '-textarea');
        if (textarea) {
    ...
    async saveTextFile(editorId) {
        ...
        try {
            ...
            this.updateEditorStatus(editorId, 'Enregistrement en cours...', 'info');
            ...
            if (response.ok) {
                ...
                this.updateEditorSaveButton(editorId);
                this.updateEditorStatus(editorId, 'Enregistré', 'success');
            ...
            this.updateEditorStatus(editorId, 'Erreur d\'enregistrement', 'error');
            if (typeof notifications !== 'undefined') {
    ...
    closeTextEditor(editorId) {
        ...
        this.closeWindow(editorId);
    },

    _buildEditorHTML(editorId, filename) {
        return `
            <div class="viewer-header viewer-header-editor" style="background: linear-gradient(135deg, #667eea 0%, #764ba2 100%);">
                ...
                <div class="flex gap-2">
                    <button onclick="event.stopPropagation(); floatingViewer.saveTextFile('${editorId}')" 
                    ...
                    <button onclick="event.stopPropagation(); floatingViewer.toggleFullscreen('${editorId}')" 
                            class="fullscreen-btn" title="Plein écran">
                        <i class="fas fa-expand"></i>
                    </button>
                    <button onclick="event.stopPropagation(); floatingViewer.minimizeWindow('${editorId}')" 
                            class="minimize-btn" title="Réduire">
                        <i class="fas fa-minus"></i>
                    </button>
                    <button onclick="event.stopPropagation(); floatingViewer.closeTextEditor('${editorId}')" title="Fermer">
            ...
            <div class="editor-toolbar" style="padding: 8px 12px; background: #f8fafc; border-bottom: 1px solid #e2e8f0; display: flex; align-items: center; gap: 8px; flex-wrap: wrap;">
                <button onclick="event.stopPropagation(); document.execCommand('undo')" 
                        class="toolbar-btn" title="Annuler (Ctrl+Z)">
                    <i class="fas fa-undo"></i>
                </button>
                <button onclick="event.stopPropagation(); document.execCommand('redo')" 
                ...
                <select id="${editorId}-font-size" 
                        onchange="floatingViewer.changeEditorFontSize('${editorId}', this.value)"
            ...
            <div class="viewer-content viewer-content-editor" style="padding: 0; background: white;">
                <textarea id="${editorId}-textarea" 
                          spellcheck="false"
                          placeholder="Chargement du contenu..."
                          style="width: 100%; height: 100%; padding: 16px; font-family: 'Consolas', 'Monaco', 'Courier New', monospace; font-size: 14px; line-height: 1.6; border: none; outline: none; resize: none; background: white; color: #1e293b; tab-size: 4;"></textarea>
            </div>
            ...
            <div class="viewer-footer" style="padding: 8px 12px; background: #f8fafc; border-top: 1px solid #e2e8f0; display: flex; justify-content: space-between; align-items: center; font-size: 11px; color: #64748b;">
                ...
                <div style="display: flex; gap: 8px; align-items: center;">
                    <button onclick="event.stopPropagation(); floatingViewer.toggleMagnetic('${editorId}')" 
